...
public class HarddiskPlugin extends Plugin{
    ...
    public ModuleExposedData exposeData() {
        ...
        data.addText(humanReadableByteCount(usedSpace, root.getTotalSpace(), true));
        return data;
    }
    ...
    private  String humanReadableByteCount(long maxBytes, boolean si) {
        int unit = si ? 1000 : 1024;
        if (maxBytes < unit)
            return maxBytes + " B";
        int exp = (int) (Math.log(maxBytes) / Math.log(unit));
